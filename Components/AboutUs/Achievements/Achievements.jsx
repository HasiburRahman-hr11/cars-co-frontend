...
const achievementsList = [
  ...
  },
  {
    ...
    description:
      "Trusted by over 32,000 customers nationwide, each experience reflects our promise of luxury, satisfaction, and long-lasting trust.",
  },
  {
    title: `<b>12000+</b> Luxury Cars Delivered`,
    thumbnail: "/images/about-us/achievement-img-5.webp",
    description:
      "With over 12,000 cars delivered, we proudly connect enthusiasts to their dream luxury vehicles across India’s roads.",
  },
  {
    title: `<b>300+</b> Verified Dealers`,
    thumbnail: "/images/about-us/achievement-img-6.webp",
    description:
      "A strong network of verified dealers ensures authenticity, transparency, and fair value in every transaction we complete.",
  },
];
        ...
        <div className="max-w-[540px] mx-auto md:max-w-none md:flex md:justify-between lg:w-[75%] lg:ml-auto lg:mr-0 lg:translate-y-[-13rem] xl:w-[78%] 1xl:translate-y-[-16rem] about-achievement-list">
          <div className="md:w-[48%] lg:w-[47%]">
            {leftCol.map((item, index) => (
              <div key={index} className="mt-[4rem] xl:mt-[6rem]">
                <h4
                  className="text-[2rem] text-[#4B4B4B] font-[500] [&>b]:font-[700] [&>b]:text-black xl:text-[3.8rem] xl:leading-[1.2] 1xl:text-[4.5rem] lg:[&>b]:block "
                  dangerouslySetInnerHTML={{ __html: item.title }}
                ></h4>
                <img
                  src={item.thumbnail}
                  alt="Thumbnail"
                  className="block object-cover rounded-[1rem] mt-[1.5rem] 1xl:mt-[3rem] xl:rounded-[2rem]"
                />
                <p className="mt-[1.5rem] text-[#636363] xl:text-[2.2rem] xl:mt-[2rem] 1xl:mt-[2.5rem]">
                  {item.description}
                </p>
              </div>
            ))}
          ...
          <div className="md:w-[48%] lg:w-[47%] lg:translate-y-[-19rem] xl:translate-y-[-40rem] 1xl:translate-y-[-42rem]">
            {rightCol.map((item, index) => (
              <div key={index} className="mt-[4rem] xl:mt-[6rem]">
                <h4
                  className="text-[2rem] text-[#4B4B4B] font-[500] [&>b]:font-[700] [&>b]:font-[urbanist] [&>b]:text-black xl:text-[3.8rem] xl:leading-[1.2] 1xl:text-[4.5rem] lg:[&>b]:block urbanist"
                  dangerouslySetInnerHTML={{ __html: item.title }}
                ></h4>
                <img
                  src={item.thumbnail}
